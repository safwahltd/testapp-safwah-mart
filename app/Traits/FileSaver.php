...
trait FileSaver
{
    ...
    */
    public function uploadFileWithResize($file, $model, $database_field_name, $basePath, $width, $height)
    {
        if ($file) {
            ...
            try {
                ...
                $image_name     = $model->id . time() . '-' . rand(11111, 999999) . '.' . $file->getClientOriginalExtension();
    ...
    public function uploadFileWithResizeForProduct($file, $model, $database_field_name, $basePath, $width, $height)
    {
        if ($file) {
            ...
            try {
                ...
                // $model->update([$database_field_name => ($basePath . '/' . $image_name)]);
                Image::make($file)->encode('png', 90)->save(public_path($basePath) . $image_name);
