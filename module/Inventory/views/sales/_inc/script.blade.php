...
<script>
    ...
    function calculateRowTotal(obj)
    {
        ...
        let pdtTotalDiscountAmount = quantity != 0 ? parseFloat( (salePrice * pdtDiscountPercent) / 100) : 0;

        let pdtTotalVatAmount      =  ((total - (pdtTotalDiscountAmount * (quantity / measurement_value))) * vatPercent)/100;
...
<script>
    ...
    function checkItemExistOrNot(obj)
    {
        ...
        let isLiveSearch  = $(obj).closest('tr').find('.is-live-search').val();
        ...
        if(variationId != '' && is_variation_obj == 'true' && is_measurement_obj == 'false'){
            if(isLiveSearch == 'true'){
            ...
            $("#invoiceSaleTable").append(
                `<tr class="new-tr">
                    ...
                    <th width="25%" style="position: relative;">
                        ...
                        <input type="hidden" class="is-live-search" value="true">
    ...
    function checkUser(obj){
        ...
        .then(function (response) {
            ...
            if(response.data == 1){
                ...
                $('#customerEditFormSubmitButton').prop("disabled",true);
                $('#existPhone').html('Already Exist');

            }else{

                $('#customerEditFormSubmitButton').prop("disabled",false);
                $('#existPhone').html('');
...
<script>
    ...
    function addAccount()
    {
        ...
        // getEcomAccountsByWarehouse()
        $('#payment-table tr:last').find('.select2').select2();
        // $('.select2').select2()
