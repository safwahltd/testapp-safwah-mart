...
@section('page-header')
    Add Delivery Man New Time Slot
@stop
        ...
        <div class="col-12">
            ...
            <div class="widget-body">
                <div class="widget-main">
                    <form class="form-horizontal mt-2" action="{{ route('inv.delivery-man-time-slots.store') }}" method="POST">
                        @csrf
                            ...
                            <div class="col-sm-4 @error('time_slot_id') has-error @enderror">
                                <select name="time_slot_id" id="time_slot_id" class="form-control select2" data-placeholder="--- Select Time Slot ---" style="width: 100%">
                                    ...
                                    @foreach ($timeSlots as $item)
                                        <option value="{{ $item->id }}">{{ $item->name }} ({{ $item->starting_time }} - {{ $item->ending_time }})</option>
                                    @endforeach
                                </select>
                                @error('time_slot_id')
