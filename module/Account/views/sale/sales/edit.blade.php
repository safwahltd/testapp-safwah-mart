            ...
            <div class="widget-box widget-color-white ui-sortable-handle clearfix" id="widget-box-7">
                ...
                <div class="widget-body">
                    <div class="widget-main">
                        <form class="form-horizontal" action="{{ route('acc-sales.update', $sale->id) }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select required name="customer_id" id="form_field" class="chosen-select-100-percent" data-placeholder="- Select Customer -">
                                            ...
                                            @foreach($customers as $id => $name)
                                                <option value="{{ $id }}" {{ old('customer_id', $sale->customer_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <table id="myTable" class="table table-bordered order-list">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="col-sm-12 prod-price">
                                                                <select required name="product_id[]" onchange="enableQty('salePrice-input', 'quantity-enable', this)" class="input-qty chosen-select-100-percent" data-placeholder="- Select Product -">
                                                                    <option></option>
                                                                    @foreach($products as $prod)
                                                                    <option value="{{ $prod->id }}" data-price="{{ $prod->sale_price }}" {{ old('product_id')[$key] == $prod->id ? 'selected' : '' }}>{{ $prod->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                        </td>
                                                ...
                                                @foreach($sale->details as $detail)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="col-sm-12 prod-price">
                                                                <select required name="product_id[]" onchange="enableQty('salePrice-input', 'quantity-enable', this)" class="input-qty chosen-select-100-percent" data-placeholder="- Select Product -">
                                                                    <option></option>
                                                                    @foreach($products as $prod)
                                                                    <option value="{{ $prod->id }}" {{ $detail->product_id == $prod->id ? 'selected' : '' }} data-price="{{ $prod->selling_price }}">{{ $prod->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            ...
                                                            <input name="sale_price[]" type="number" value="{{ $detail->price }}" class="form-control only-number text-right salePrice-input" />
                                                            @error('debit')
                                                                <span class="text-danger"> {{ $message }}</span>
                                                            @enderror
                                                        </td>
                                                        <td>
                                                            <input name="quantity[]" required type="number" value="{{ $detail->quantity }}" class="form-control text-right only-number quantity calculate-total quantity-enable" />
                                                            @error('credit')
                                                                <span class="text-danger"> {{ $message }}</span>
                                                            @enderror
                                                        </td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td>
                                                    <input readonly name="qty_total" value="{{ old('qty_total', $sale->qty_total) }}" class="quantityTotal text-right form-control">
                                                </td>
                                                <td>
                                                    <input readonly name="qty_amount" value="{{ old('qty_amount', $sale->qty_amount) }}" class="itemTotal text-right form-control">
                                                </td>
                                                <td>
                                        ...
                                        <div class="col-md-5 pull-right">
                                            <div class="form-group">
                                                ...
                                                <div class="col-xs-12 col-sm-8 @error('cost') has-error @enderror">
                                                    <input name="discount_amount" value="{{ old('discount_amount', $sale->discount_amount) }}" value="0.00" disabled class="discount only-number calculate-total dicount-enable text-right form-control">
                                                    @error('cost')
                                                    <span class="text-danger">{{ $message }}</span>
                                                    @enderror
                                                </div>
                                        ...
                                        <div class="col-md-5 pull-right">
                                            <div class="form-group">
                                                ...
                                                <div class="col-xs-12 col-sm-8 @error('end_date') has-error @enderror">
                                                    <input name="total_amount" value="{{ old('total_amount', $sale->total_amount) }}" readonly class="totalAmount text-right form-control">
                                                    @error('end_date')
                                                    <span class="text-danger">{{ $message }}</span>
                                                    @enderror
                                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-5 pull-right">
                                            <div class="form-group">
                                                ...
                                                <div class="col-xs-12 col-sm-8 @error('end_date') has-error @enderror">
                                                    <input name="paid_amount" value="{{ old('paid_amount', $sale->paid_amount) }}" autocomplete="off" class="paidAmount only-number calculate-paid total-credit text-right form-control">
                                                    @error('end_date')
                                                    <span class="text-danger">{{ $message }}</span>
                                                    @enderror
                                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-5 pull-right">
                                            <div class="form-group">
                                                ...
                                                <div class="col-xs-12 col-sm-8 @error('end_date') has-error @enderror">
                                                    <input name="due_amount" value="{{ old('due_amount', $sale->due_amount) }}" readonly class="dueAmount total-credit text-right form-control">
                                                    @error('end_date')
                                                    <span class="text-danger">{{ $message }}</span>
                                                    @enderror
                                                </div>
    ...
    <script>
        ...
        const rowItem = `<tr>
                            ...
                            <td>
                                ...
                                <input name="sale_price[]" type="text" class="form-control only-number text-right salePrice-input" />
                                @error('debit')
                                    <span class="text-danger"> {{ $message }}</span>
                                @enderror
                            </td>
