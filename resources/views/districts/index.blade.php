...
@section('content')
    <div class="page-header">
        ...
        <div class="btn-group">
            <a class="btn btn-sm btn-primary" href="{{ route('districts.create') }}">
    ...
    @include('districts.show-modal')

    <div class="row">
        <div class="col-xs-12">
            <div class="table-responsive">
                <form action="{{ route('update-districts-data') }}" method="POST">
                    @csrf
                        ...
                        <tbody>
                            @forelse($districts as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->shipping_cost ? number_format($item->shipping_cost, 2) : 'N\A' }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-corner">
                                            ...
                                            data-name="{{ $item->name }}"
                                            data-shipping-cost="{{ number_format($item->shipping_cost, 2) }}"
                                            data-free_delivery_amount="{{ number_format($item->free_delivery_amount, 2) }}"
                                            ...
                                            <!-- EDIT -->
                                            <a href="{{ route('districts.edit', $item->id) }}" class="btn btn-xs btn-success" title="Edit">
                                            ...
                                            @if($countDeliveryManInOrder < 1)
                                                <button class="btn btn-xs btn-danger" title="Delete" onclick="delete_item('{{ route('districts.destroy', $item->id) }}')" type="button">
                ...
                @include('partials._paginate', ['data'=> $districts])
            </div>
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function () {
            $('#showDetails').on('show.bs.modal', function (event) {
                ...
                let free_delivery_amount    = button.data('free_delivery_amount');
                let status                  = button.data('status');
                let modal                   = $(this)

                modal.find('.modal-body #name').val(name);
                modal.find('.modal-body #shipping_cost').val(shipping_cost);
                modal.find('.modal-body #free_delivery_amount').val(free_delivery_amount);
